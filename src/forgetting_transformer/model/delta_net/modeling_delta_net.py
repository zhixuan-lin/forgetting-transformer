...
from .delta_net_layer import DeltaNet
...
class DeltaNetMLP(nn.Module):
    ...
    def forward(self, x):
        if self.norm_first:
            gate, y = self.norm(x, self.gate_proj.weight.to(x.dtype), self.gate_proj.bias.to(x.dtype) if self.gate_proj.bias is not None else None).chunk(2, -1)
        else:
